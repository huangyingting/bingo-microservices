...
// default is 10
func Attempts(attempts uint32) Option {
...
func BackOffDelay(n uint32, _ error, config *Config) time.Duration {
	...
	if config.maxBackOffN == 0 {
		...
		config.maxBackOffN = max - uint32(math.Floor(math.Log2(float64(config.delay))))
	}
...
func FixedDelay(_ uint32, _ error, config *Config) time.Duration {
	return config.delay
}

// RandomDelay is a DelayType which picks a random delay up to config.maxJitter
func RandomDelay(_ uint32, _ error, config *Config) time.Duration {
...
func CombineDelay(delays ...DelayTypeFunc) DelayTypeFunc {
	...
	return func(n uint32, err error, config *Config) time.Duration {
		var total uint64
